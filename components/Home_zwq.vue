...
<div class=".container-fluid">
  ...
  <div class="nav">
    <van-row>
      ...
      <div  v-if="isdl">
        <van-col span="6  " >
          <router-link :to="{}" class="color1" >登录</router-link>
          <router-link :to="{}" class="color1">注册</router-link>
        </van-col>
      </div>
  ...
  <div class="guesstext guess" >
      <van-row >
        <van-col span="22"><span class="guess_yes">
          <router-link :to="{path:'/sousuoShi',query:{name1:arrguess}}">{{arrguess.name}}</router-link></span></van-col>
        <van-col span="2" >
...
<!--循环获取热门城市-->
    <ul>
      <li  class="hot_cs" v-for="(n ,index) in arrHot " :key="index"><router-link :to="{path:'/sousuoShi',query:{name2:n}}"  class="color"><span>{{n.name}}</span></router-link></li>
    </ul>
  </div>
    ...
    <div v-for="(value,key,index ) in arrgroup" :key="index">
      ...
      <ul>
        <li v-for="(n,index) in value" class="alllist":key="index ">
         <router-link :to="{path:'/sousuoShi',query:{name:n}}" class="alla">{{n.name}}</router-link>
          <!--query:{id:n.id}-->
        </li>
      </ul>
    </div>
...
<script>
  import Vue from "vue"
    export default {
      ...
      mounted(){
          //获取搜索餐馆
        Vue.axios.get("http://api.kuailezan.com/api/open/create_task").then((reslut)=>{
