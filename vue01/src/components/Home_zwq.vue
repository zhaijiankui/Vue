  ...
  <div class="guesstext guess" >
      <van-row >
        <van-col span="22"><span class="guess_yes">
          <router-link :to="{path:'/child1',query:{name:arrguess.name,id:arrguess.id }}">{{arrguess.name}}</router-link></span></van-col>
        <van-col span="2" >
...
<!--循环获取热门城市-->
    <ul>
      <li  class="hot_cs" v-for="(n ,index) in arrHot " :key="index"><router-link :to="{path:'/child1',query:{name1:n.name,name3:n.id}}"  class="color"><span>{{n.name}}</span></router-link></li>
    </ul>
  </div>
    ...
    <div v-for="(value,key,index ) in arrgroup" :key="index">
      ...
      <ul>
        <li v-for="(n,index) in value" class="alllist":key="index ">
         <router-link :to="{path:'/child1',query:{name2:n.name,name4:n.id}}" class="alla">{{n.name}}</router-link>
        </li>
      </ul>
    </div>
